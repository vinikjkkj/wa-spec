 ...
 *
 * Downloads every loaded JS bundle from web.whatsapp.com to disk and emits a
 * manifest.json. That's it — no extraction, no parsing. Per-domain extractors
 ...
 *   //   dump.waVersion            string | null
 *   //   dump.paths.raw            absolute path to <out>/raw/<version>/
 *   //   dump.paths.manifest       absolute path to <out>/manifest.json
 */
...
async function fetchBundles(opts = {}) {
    ...
    try {
        ...
        // Discover every JS bundle URL — not just the ones the SPA already
        // loaded. Meta inlines the full resource manifest inside
        // `<script data-sjs>` blocks (rsrcMap), and also references chunks via
        // preload links / inline `<script>` literals / performance entries.
        //
        // (Discovery technique cribbed from vinikjkkj/wa-diff/fetch.js.)
        ...
        // Try to read the WA version (best-effort)
        ...
        // Fetch each bundle in-page (CORS-safe — same origin as the SPA)
